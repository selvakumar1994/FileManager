...
import com.amaze.file.asynchronous.services.ZipService;
...
import com.amaze.file.filesystem.Operations;
    ...
    void mkfile(final OpenMode openMode, final String path, final MainFragment ma) {
        mk(R.string.newfile,  NEW_FILE_TXT_EXTENSION, (dialog, which) -> {
            ...
            if (isValidFilename && text.length() > 0) {
                ...
                if(text.startsWith(".") && !prefs.getBoolean(com.amaze.file.fragments.preference_fragments.PreferencesConstants.PREFERENCE_SHOW_HIDDENFILES, false)){
                    return new WarnableTextInputValidator.ReturnState(
